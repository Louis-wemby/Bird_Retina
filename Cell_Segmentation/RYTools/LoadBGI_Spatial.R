...
parser <- ArgumentParser(description = "scgem to rds")
parser$add_argument("--infile", type = "character", required = TRUE, help = "Path to the scgem.csv.gz file")
parser$add_argument("--outfile", type = "character", required = TRUE, help = "Path to save the output file")
parser$add_argument("--snid", type = "character", required = TRUE, help = "SN id")
parser$add_argument("--csv", type = "logical", default = TRUE, help = "Is the input a CSV file? (default: TRUE)")
parser$add_argument("--cell_mask", type = "logical", default = TRUE, help = "Use cell mask? (default: TRUE)")
parser$add_argument("--bin_data", type = "logical", default = FALSE, help = "Use bin data? (default: FALSE)")
args <- parser$parse_args()

infile = args$infile
outfile = args$outfile
snid = args$snid
csv = ifelse(is.null(args$csv), TRUE, args$csv)
cell_mask = ifelse(is.null(args$cell_mask), TRUE, args$cell_mask)
bin_data = ifelse(is.null(args$bin_data), FALSE, args$bin_data)

LoadBGI_Spatial <- function(infile,
                            outfile,
                            bin_data = F,
                            bin_size = 50,
                            cell_mask = T,
  ...
  print('mark')}
    ...
    colnames(dat)[1:5] <- c("geneID", "geneName", "x", "y", "MIDCount")
    if (bin_data) {
        rowname_style <- "BIN"
        ...
        # Modify colnames，use MIDCount
        dat <- as.data.table(dat)
        dat <- dat[, sum(MIDCount), by = .(geneID, geneName, x, y)]
        dat$bin_ID <- max(dat$x) * (dat$y - 1) + dat$x
    ...
    if (cell_mask) {
        rowname_style <- "CELL"
        # Add the column 'geneName'
        colnames(dat)[1:7] <- c("geneID", "geneName", "x", "y", "MIDCount", "ExonCount", "label")
        if (delete_bg) {
	dat <- subset(dat, label > 0)}
        ...
        dat <- as.data.table(dat)
        dat_bkp <- dat
        dat_bkp <- dat_bkp[, sum(MIDCount), by = .(geneID, geneName, x, y)]
        ...
        # Modify colnames，use MIDCount
        dat <- dat[, sum(MIDCount), by = .(geneID, geneName, x, y, label)]
...
result <- LoadBGI_Spatial(infile=infile, outfile=outfile, pro_name=snid)
